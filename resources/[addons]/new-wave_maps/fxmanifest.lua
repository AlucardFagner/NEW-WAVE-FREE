fx_version 'adamant'
game 'gta5'

this_is_a_map 'yes'

client_scripts { 
    'hansolo/*.lua'
}

files {
    'gabz_timecycle_mods_1.xml',
  --  'interiorproxies.meta',
    'ex_int_warehouse_small_dlc.ytyp',
    'int_industrial.ytyp',
    'vw_prop_vw_turntable.ytyp',
    'vw_prop_vw_tables.ytyp',
    'vw_prop_vw_storage.ytyp',
    'int_services.ytyp',
    'vw_prop_vw_screens.ytyp',
    'int_retail.ytyp',
    'int_residential.ytyp',
    'casino_props.ytyp',
    'vw_prop_vw_machinery.ytyp',
    'int_lev_des.ytyp',
    'int_industrial.ytyp',
    'vw_prop_vw_door_break.ytyp',
    'vw_prop_vw_disposal.ytyp',
   -- 'int_corporate.ytyp',
    'vw_prop_vw_collectibles.ytyp',
    'vw_prop_vw_casinochips.ytyp',
    'vw_prop_vw_casino.ytyp',
    'vw_prop_vw_cards.ytyp',
    'vw_prop_vw_cabinets.ytyp',
    'vw_prop_vw_casino_art_05.ytyp',
    'vw_prop_vw_casino_art_04.ytyp',
    'vw_prop_vw_casino_art_03.ytyp',
    'vw_prop_vw_casino_art_02.ytyp',
    'vw_prop_vw_casino_art_01.ytyp',
    'vw_prop_vw_animated_walls.ytyp',
    'int_medical.ytyp',
    'vw_prop_vw_accs.ytyp',
    'vw_prop_vw_accs_01.ytyp',
    'gusepe_timecycle_mods_1.xml',
    'timecycle_mods_1.xml',
    'kambi_canyon.ytyp'
} 

data_file 'DLC_ITYP_REQUEST' 'kambi_canyon.ytyp'
data_file 'TIMECYCLEMOD_FILE' 'gabz_timecycle_mods_1.xml'
--data_file 'INTERIOR_PROXY_ORDER_FILE' 'interiorproxies.meta'
data_file 'DLC_ITYP_REQUEST' 'stream/[arcadebar]/props/arcade_props.ytyp'
data_file 'DLC_ITYP_REQUEST' 'ex_int_warehouse_small_dlc.ytyp'
data_file 'DLC_ITYP_REQUEST' 'int_industrial.ytyp'
data_file 'DLC_ITYP_REQUEST' 'stream/vesp_props.ytyp'
data_file 'DLC_ITYP_REQUEST' 'stream/v_int_40.ytyp'
data_file 'DLC_ITYP_REQUEST' 'stream/mansao-madrasso/props.ytyp'
data_file 'TIMECYCLEMOD_FILE' 'casino_timecyc.xml'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_turntable.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_tables.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_storage.ytyp'
data_file 'DLC_ITYP_REQUEST' 'int_services.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_screens.ytyp'
data_file 'DLC_ITYP_REQUEST' 'int_retail.ytyp'
data_file 'DLC_ITYP_REQUEST' 'int_residential.ytyp'
data_file 'DLC_ITYP_REQUEST' 'casino_props.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_machinery.ytyp'
data_file 'DLC_ITYP_REQUEST' 'int_lev_des.ytyp'
data_file 'DLC_ITYP_REQUEST' 'int_industrial.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_door_break.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_disposal.ytyp'
--data_file 'DLC_ITYP_REQUEST' 'int_corporate.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_collectibles.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_casinochips.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_casino.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_cards.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_cabinets.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_casino_art_05.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_casino_art_04.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_casino_art_03.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_casino_art_02.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_casino_art_01.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_animated_walls.ytyp'
data_file 'DLC_ITYP_REQUEST' 'int_medical.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_accs.ytyp'
data_file 'DLC_ITYP_REQUEST' 'vw_prop_vw_accs_01.ytyp'
data_file 'TIMECYCLEMOD_FILE' 'gusepe_timecycle_mods_1.xml'
data_file 'TIMECYCLEMOD_FILE' 'timecycle_mods_1.xml'
data_file 'DLC_ITYP_REQUEST' 'stream/vw_prop_vw_casino_art_02.ytyp'
