...
function cRP.paymentMethod(locate)
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		local randPrice = math.random(650,850)

		vRP.upgradeStress(user_id,3)
		TriggerEvent("Wanted",source,user_id,30)
		vRP.generateItem(user_id,"dollarsz",parseInt(randPrice),true)

		local identity = vRP.userIdentity(user_id)
		if identity["locate"] ~= locate then
			vRP.generateItem(user_id,"dollarsz",parseInt(randPrice * 0.1),true)
		end
	end
end