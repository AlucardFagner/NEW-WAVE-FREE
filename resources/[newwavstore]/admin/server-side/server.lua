    ...
    if user_id then
        if vRP.hasGroup(user_id, "Admin") then
            if args[1] and args[2] and itemBody(args[1]) ~= nil then
                ...
                -- Log para o Discord
                local message = string.format("Admin %s deu %sx %s ao jogador %s **Horário:** %s ", GetPlayerName(source), args[2], args[1], user_id, os.date("%H:%M:%S"))
                TriggerEvent("discordLogs","Item", message) -- Substitua "admin" pelo canal de registro apropriado

                -- Você também pode ajustar a mensagem de log conforme necessário.
...
RegisterCommand("nc",function(source,args,rawCommand)
	...
	if user_id then
		if vRP.hasGroup(user_id,"Moderator") then
			...
			local alucard = string.format("**Admin:** %s **DEU NC** \n**ID:** %s \n**HORARIO:** %s", GetPlayerName(source), user_id, os.date("%H:%M:%S"))
			TriggerEvent("discordLogs","nc", alucard)
