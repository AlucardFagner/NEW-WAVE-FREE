...
setInterval(() => {
    if (navigator.onLine != Networked){
      ...
      $.post("http://inventory/Network",JSON.stringify({ status: Networked }));
    }
},500)
// -------------------------------------------------------------------------------------------
...
const updateDrag = () => {
    ...
    $(".populated").tooltip({
        create: function(event, ui) {
            ...
            $(this).tooltip({
                content: `<item>${name}</item>${description !== "undefined" ? "<br><description>"+description+"</description>":""}<br><legenda>Tipo: <r>${type}</r> <s>|</s> Máximo: <r>${max !== "undefined" ? max:"S/L"}</r> ${serial != "undefined" ? `<s>|</s> Serial: <r>${serial}</r>` : "" }</legenda>`,
...
const updateMochila = () => {
        $.post("http://inventory/requestInventory", JSON.stringify({}), (data) => {
            $("#weightTextLeft").html(`${(data["invPeso"]).toFixed(2)}/${(data["invMaxpeso"]).toFixed(2)}KG`);
            ...
            for (let x = 1; x <= data["invMaxpeso"]; x++) {
                ...
                if (data["inventario"][slot] !== undefined) {
                    ...
                    const item = `<div class="item populated" title="" data-max="${v["max"]}" data-type="${v["type"]}" data-description="${v["desc"]}" style="background-image: url('images/${v["index"]}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v["amount"]}" data-peso="${v["peso"]}" data-item-key="${v["key"]}" data-name-key="${v["name"]}" data-serial="${v["serial"]}" data-slot="${slot}">
					<div class="top">
						<div class="itemWeight">${(v["peso"] * v["amount"]).toFixed(2)}</div>
						<div class="itemAmount">${formatarNumero(v["amount"])}x</div>
					</div>

					<div class="durability" style="width: ${actualPercent == 1 ? "100":actualPercent}%; background: ${actualPercent == 1 ? "#05f1db":colorPicker(actualPercent)};"></div>
            ...
            for (let x = 1; x <= 30; x++) {
                ...
                if (nameList2[x - 1] !== undefined) {
					...
					<div class="durability" style="width: ${actualPercent == 1 ? "100":actualPercent}%; background: ${actualPercent == 1 ? "#c0392b":colorPicker(actualPercent)};"></div>
					<div class="nameItem">${v["name"]}</div>
				</div>`;
