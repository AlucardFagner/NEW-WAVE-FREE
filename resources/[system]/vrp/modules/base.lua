...
-- DISCORDHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webHook = "https://discord.com/api/webhooks/1147585832157458523/zIMbLZxPUtccJx5iIGPLCLUxB_yoRIN0ZgKDtE4ec-cyZHAWh2b81DEpXnI03dTvHQI7"
-----------------------------------------------------------------------------------------------------------------------------------------
-- MYSQL
-----------------------------------------------------------------------------------------------------------------------------------------
local mysqlDriver
...
-- CHECKBANNED
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.checkBanned(discord)
	local consult = vRP.query("banneds/getBanned",{ discord = discord })
	if consult[1] then
		if consult[1]["time"] >= os.time() then
			vRP.execute("banneds/removeBanned",{ discord = discord })
			return false
		end
...
-- INFOACCOUNT
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.infoAccount(discord)
	local infoAccount = vRP.query("accounts/getInfos",{ discord = discord })
...
AddEventHandler("Queue:playerConnecting",function(source,identifiers,deferrals)
	...
	if discord then
		if not vRP.checkBanned(discord) then
			local infoAccount = vRP.infoAccount(discord)
			if infoAccount then
				if infoAccount["whitelist"] then
					deferrals.done()
				else
					deferrals.done("Seu passaporte não esta autorizado! Entre no nosso Discord: https://discord.gg/6qKasA3Jgk/ para mais informações, Passaporte: "..discord)
				end
			else
				vRP.execute("accounts/newAccount",{ discord = discord })
				deferrals.done("Envie na sala liberação: "..discord)
			end
		else
			deferrals.done("Você está banido da cidade.")
		end
	else
		deferrals.done("Ocorreu um problema ao conectar com seu discord.")
	end
...
function vRP.characterChosen(source,user_id,model,locate)
	...
	local infoAccount = vRP.infoAccount(identity["discord"])
	if infoAccount then
	...
	if Identities ~= identity["discord"] then
