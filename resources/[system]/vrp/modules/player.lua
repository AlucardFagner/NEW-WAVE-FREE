...
AddEventHandler("playerConnect",function(user_id,source)
	...
	if dataTable then
		...
		if identity then
			TriggerClientEvent("vRP:playerActive",source,user_id,identity["name"].." "..identity["name2"],GetNumPlayerIndices())
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	if user_id then
		...
		if dataTable then
			if dataTable["salary"] ~= nil then
				TriggerClientEvent("Notify",source,"verde","Recebeu <b>R$"..parseFormat(dataTable["salary"]).."</b> em sua conta bancária.",5000)
