...
vRP.prepare("characters/getCharacters","SELECT * FROM summerz_characters WHERE discord = @discord and deleted = 0")
...
vRP.prepare("characters/lastCharacters","SELECT id FROM summerz_characters WHERE discord = @discord ORDER BY id DESC LIMIT 1")
vRP.prepare("characters/countPersons","SELECT COUNT(discord) as qtd FROM summerz_characters WHERE discord = @discord and deleted = 0")
vRP.prepare("characters/newCharacter","INSERT INTO summerz_characters(discord,name,name2,locate,sex,phone,serial,blood,bank) VALUES(@discord,@name,@name2,@locate,@sex,@phone,@serial,@blood,@bank)")
-----------------------------------------------------------------------------------------------------------------------------------------
-- BANK
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- ACCOUNTS
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.prepare("accounts/getInfos","SELECT * FROM summerz_accounts WHERE discord = @discord")
vRP.prepare("accounts/newAccount","INSERT INTO summerz_accounts(discord) VALUES(@discord)")
vRP.prepare("accounts/updateWhitelist","UPDATE summerz_accounts SET whitelist = 0 WHERE discord = @discord")
vRP.prepare("accounts/removeGems","UPDATE summerz_accounts SET gems = gems - @gems WHERE discord = @discord")
vRP.prepare("accounts/setPriority","UPDATE summerz_accounts SET priority = @priority WHERE discord = @discord")
vRP.prepare("accounts/infosUpdatechars","UPDATE summerz_accounts SET chars = chars + 1 WHERE discord = @discord")
vRP.prepare("accounts/infosUpdategems","UPDATE summerz_accounts SET gems = gems + @gems WHERE discord = @discord")
vRP.prepare("accounts/updatePremium","UPDATE summerz_accounts SET premium = premium + 2592000 WHERE discord = @discord")
vRP.prepare("accounts/setPremium","UPDATE summerz_accounts SET premium = @premium, priority = @priority WHERE discord = @discord")
-----------------------------------------------------------------------------------------------------------------------------------------
-- PLAYERDATA
-----------------------------------------------------------------------------------------------------------------------------------------
...
vRP.prepare("playerdata/datatable","SELECT JSON_UNQUOTE(JSON_KEYS(JSON_EXTRACT(`dvalue`, '$.perm'))) AS permissions FROM summerz_playerdata WHERE user_id = @user_id AND dkey = 'Datatable'")
-----------------------------------------------------------------------------------------------------------------------------------------
-- ENTITYDATA
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- vRP.prepare("vehicles/selectDesmanche","SELECT * FROM summerz_vehicles WHERE plate = @vehicle")
-----------------------------------------------------------------------------------------------------------------------------------------
-- PROPERTYS
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- BANNEDS
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.prepare("banneds/getBanned","SELECT * FROM summerz_banneds WHERE discord = @discord")
vRP.prepare("banneds/removeBanned","DELETE FROM summerz_banneds WHERE discord = @discord")
vRP.prepare("banneds/insertBanned","INSERT INTO summerz_banneds(discord,time) VALUES(@discord,UNIX_TIMESTAMP() + 86400 * @time)")
-----------------------------------------------------------------------------------------------------------------------------------------
-- CHESTS
-----------------------------------------------------------------------------------------------------------------------------------------
